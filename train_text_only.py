#!/usr/bin/env python3
"""
Train text-only model for disease classification
Simplified training focused on getting text to work
"""
...
from models import TextDiseaseModel

# Dummy processor for dataset compatibility
class DummyProcessor:
    def __call__(self, images, **kwargs):
        return {'pixel_values': torch.zeros(3, 512, 512)}
...
def train_text_model():
    ...
    processor = DummyProcessor()  # Dummy for compatibility
    ...
    # Create model
    print("\nCreating model...")
    model = TextDiseaseModel('microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext').to(device)
    ...
    save_dir = Path('/content/drive/MyDrive/checkpoints/text_only')
    ...
    for epoch in range(epochs):
        ...
        if avg_auc > best_auc:
            best_auc = avg_auc
            save_path = save_dir / 'text_best.pth'
            torch.save({
                'epoch': epoch,
        ...
        if (epoch + 1) % 10 == 0:
            save_path = save_dir / f'text_epoch_{epoch+1}.pth'
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
    ...
    print(f"Model saved to: {save_dir / 'text_best.pth'}")
    print("="*70)
